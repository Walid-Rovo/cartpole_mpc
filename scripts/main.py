...
def simulate(params_dict, x_0=np.array([0.5, 0, 0.0, 0, 0.36, 0.26]).reshape([-1, 1])):
    ...
    for _ in range(N_sim):
        ...
        mpc_res = controller(p=x_next, lbg=0, ubg=0, lbx=solver.lb_opt_x, ubx=solver.ub_opt_x)
...
if __name__ == "__main__":
    params_dict = {
        ...
        "MPC_xdot_bound": 50.0,
        "MPC_theta_bound": 4.0,
        "MPC_thetadot_bound": 50.0,
        "MPC_u_bound": 15,
    }
    x_0 = np.array([0.5, 0, -3, 0, 0.36, 0.26]).reshape([-1, 1])  # true states
    # x_0 = np.array([0.5, 0, -1., 0, 0.1, 0.1]).reshape([-1, 1])  # PDF states

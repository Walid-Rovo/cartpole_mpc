...
def simulate(params_dict, x_0_plant, x_0_observer):
    ...
    for k in range(N_sim):
        ...
        elif k % int(1 / params_dict["dt"]) and k < int(2 / params_dict["dt"]):
            u_k = (np.random.normal(0, 1e1) + -abs(np.random.normal(0, 1e1)) * x_next[1]).reshape(
                -1, 1
            )
        else:
        ...
        if k == int(7 / params_dict["dt"]):
            ...
            solver.setpoint[0] = 4
            solver.generate()
        elif k > int(5 / params_dict["dt"]) and k % int(7 / params_dict["dt"]) == 0:
            # Flip x setpoint
            solver.setpoint[0] *= -1
            solver.generate()
...
if __name__ == "__main__":
    ...
    res_x_mpc, res_x_mpc_full, res_x_hat, res_u_mpc, res_std_ekf = simulate(
        params_dict, x_0_plant, x_0_observer
    )
